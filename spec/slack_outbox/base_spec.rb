...
RSpec.describe SlackOutbox::Base do
  # Use OS subclass for testing since Base has abstract methods
  ...
  describe "expects" do
    describe "channel preprocessing" do
      ...
      context "with symbol channel key" do
        subject(:result) { action_class.call(channel: :slack_development, text:) }

        it "resolves to channel ID" do
          expect(client_dbl).to receive(:chat_postMessage).with(
            hash_including(channel: action_class::CHANNELS[:slack_development]),
          )
      ...
      context "with unknown symbol channel key" do
        subject(:result) { action_class.call(channel: :unknown_channel, text:) }
      ...
      context "with string channel ID" do
        subject(:result) { action_class.call(channel: "C123456", text:) }
    ...
    describe "text preprocessing" do
      ...
      context "with nil text" do
        subject(:result) { action_class.call(channel:, text: nil) }
    ...
    describe "icon_emoji preprocessing" do
      subject(:result) { action_class.call(channel:, text:, icon_emoji:) }
  ...
  describe "validation (before block)" do
    context "when content is blank" do
      subject(:result) { action_class.call(channel:) }
    ...
    context "when blocks are invalid" do
      subject(:result) { action_class.call(channel:, blocks:) }
    ...
    context "when files are provided" do
      ...
      context "with blocks" do
        subject(:result) { action_class.call(channel:, files:, blocks: [{ type: "section" }]) }
      ...
      context "with attachments" do
        subject(:result) { action_class.call(channel:, files:, attachments: [{ color: "good" }]) }
      ...
      context "with icon_emoji" do
        subject(:result) { action_class.call(channel:, files:, icon_emoji: "robot") }
      ...
      context "with text only" do
        subject(:result) { action_class.call(channel:, files:, text:) }
  ...
  describe "#call" do
    describe "posting messages" do
      subject(:result) { action_class.call(channel:, text:, blocks:, attachments:, icon_emoji:, thread_ts:) }
      ...
      context "not in production" do
        ...
        it "posts to dev channel with wrapped text" do
          expect(client_dbl).to receive(:chat_postMessage).with(
            channel: action_class::CHANNELS[:slack_development],
    ...
    describe "uploading files" do
      subject(:result) { action_class.call(channel:, files:, text: "File attached") }
    ...
    describe "error handling" do
      ...
      context "when NotInChannel error occurs" do
        subject(:result) { action_class.call!(channel:, text:) }

        it "sends error notification and re-raises" do
          error_channel = action_class::CHANNELS[:eng_alerts]
          call_count = 0
      ...
      context "when error_channel is same as target channel" do
        subject(:result) { action_class.call!(channel: action_class::CHANNELS[:eng_alerts], text:) }
  ...
  describe ".format_group_mention" do
    ...
    context "in production" do
      ...
      it "returns formatted group link for string ID" do
        result = action_class.format_group_mention("S123ABC")
    ...
    context "not in production" do
      ...
      it "returns dev group link by default" do
        ...
        expect(result).to eq("<!subteam^#{action_class::USER_GROUPS[:slack_development]}>")
      end

      it "uses custom non_production fallback if provided" do
        result = action_class.format_group_mention("S123ABC", "S_CUSTOM_DEV")
  ...
  describe "test_message_wrapper" do
    subject(:result) { action_class.call(channel:, text: "Line 1\nLine 2\nLine 3") }
